---
title: Android 12 差分包制作指南
slug: 670e43101d3344aaa8d720ddf31ac1a1
tags:
  - android
date: 2025-07-02T11:02:22.705Z
---

## 📝 完整差分包制作流程

### 核心命令

```bash
# 使用系统编译生成的工具
out/host/linux-x86/bin/ota_from_target_files \
  -v \
  --block \
  --path out/host/linux-x86/ \
  -i old_target_files.zip \
  new_target_files.zip \
  ota_update.zip
```

### 加速AB升级选项

```bash
# 添加跳过postinstall和FEC计算的参数
out/host/linux-x86/bin/ota_from_target_files \
  --disable_fec_computation \
  --skip_postinstall \
  ... # 其他参数同上
```

## ⚠️ 关键注意事项

### 1. 环境要求

- **Ubuntu 18.04** (推荐)

- Python 2.7 (Android 12仍使用Python 2)

- 已安装Android编译环境

- 已编译完整系统 (`make -j8`)

### 2. 文件准备

| 文件类型                   | 生成方式        | 说明      |
| ---------------------- | ----------- | ------- |
| old\_target\_files.zip | `make dist` | 旧版本目标文件 |
| new\_target\_files.zip | `make dist` | 新版本目标文件 |
| ota\_update.zip        | 本命令生成       | 差分包输出文件 |

### 3. 参数说明

| 参数                          | 作用            | 推荐值                   |
| --------------------------- | ------------- | --------------------- |
| `-v`                        | 详细输出          | 始终启用                  |
| `--block`                   | 使用块增量更新       | 必须启用                  |
| `--path`                    | 工具依赖路径        | `out/host/linux-x86/` |
| `-i`                        | 增量模式          | 后接旧版本zip              |
| `--disable_fec_computation` | 禁用FEC计算       | 加速制作                  |
| `--skip_postinstall`        | 跳过postinstall | 加速安装                  |

## 🔧 常见错误解决方案

### 错误：`ImportError: No module named google.protobuf`

```bash
# 安装Python 2的protobuf
sudo apt-get install python-protobuf

# 或使用pip安装
pip install protobuf==3.19.4 --user
```

### 错误：`Warning: releasetools script should be invoked as hermetic Python executable`

**正确做法**：

```bash
# 不要使用源码中的脚本
❌ ./build/tools/releasetools/ota_from_target_files ...

# 应使用编译生成的版本
✅ out/host/linux-x86/bin/ota_from_target_files ...
```

### 其他依赖问题

```bash
# 安装所有必要依赖
sudo apt-get install \
  python-dev \
  python-protobuf \
  python-pil \
  python-six \
  python-zlib \
  libssl-dev
```

## ⚙️ 高级配置选项

### 1. 密钥配置

```bash
# 使用自定义签名密钥
ota_from_target_files \
  --package_key build/make/target/product/security/testkey \
  ... # 其他参数
```

### 2. 增量策略优化

```bash
# 仅更新特定分区
ota_from_target_files \
  --dynamic_partition_list system,vendor \
  ... # 其他参数
```

### 3. 添加额外文件

```bash
# 包含额外脚本
ota_from_target_files \
  --post_install_script device/extra/update-script.sh \
  ... # 其他参数
```

## 📊 不同编译模式对比

| 编译类型                  | 制作命令                                  | 输出文件              | 适用场景    |
| --------------------- | ------------------------------------- | ----------------- | ------- |
| **Split build**       | `make dist` + `ota_from_target_files` | target\_files.zip | 正式OTA发布 |
| **Full build**        | `make otapackage`                     | 完整OTA包            | 本地测试    |
| **Incremental build** | `make incremental_ota`                | 增量包               | 快速迭代    |

## 💡 性能优化技巧

### 1. 并行处理

```bash
# 使用多核加速
ota_from_target_files \
  --worker_threads $(nproc) \
  ... # 其他参数
```

### 2. 缓存优化

```bash
# 设置临时文件目录
export TMPDIR=/ssd/tmp  # 使用SSD加速
mkdir -p $TMPDIR
```

### 3. 内存优化

```bash
# 增加Java堆大小
export JAVA_OPTS="-Xmx8g -Xms2g"
```

## ⚠️ 常见问题排查

### 问题：差分包生成失败

**解决方案**：

1. 检查版本连续性：

   ```bash
   # 确保是连续版本
   grep ro.build.fingerprint old/build.prop
   grep ro.build.fingerprint new/build.prop
   ```

2. 验证分区大小：

   ```bash
   # 检查分区大小是否一致
   grep partition_size old/META/misc_info.txt
   grep partition_size new/META/misc_info.txt
   ```

### 问题：设备端更新失败

**检查点**：

1. 设备存储空间是否充足

2. 当前系统版本是否匹配旧版本

3. Bootloader版本是否兼容

4. 签名密钥是否匹配

## 📱 差分包应用验证

```bash
# 推送到设备测试
adb push update.zip /sdcard/

# 进入recovery模式
adb reboot recovery

# 应用更新
adb sideload update.zip
```

> 💡 **专业提示**：对于生产环境，建议添加版本验证步骤：
>
> ```bash
> # 验证版本连续性
> ota_from_target_files --verify \
>   -i old-target_files.zip \
>   new-target_files.zip
> ```
