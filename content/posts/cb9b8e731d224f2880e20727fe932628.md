---
title: Android 15 屏蔽指定应用安装的修改记录
...
# Android 15 屏蔽指定应用安装的修改记录

 

### ✍️ 修改目的

在 Android 15 系统中，实现对特定应用的安装屏蔽，使其无法通过手动方式安装。
...
```diff
--- a/kte_project/android/public/system/frameworks/base/services/core/java/com/android/server/pm/PackageInstallerService.java
+++ b/kte_project/android/public/system/frameworks/base/services/core/java/com/android/server/pm/PackageInstallerService.java
...
- 在创建安装会话时，系统会调用 `shouldBlockInstallation()` 方法检查包名。

- 若包名包含 `"org.epcdiy.memory"`，则会抛出 `SecurityException`，阻止安装流程。

- 该检查仅针对手动安装场景，系统预装或特权安装可能不受影响。
...
- 当前屏蔽规则为硬编码，后续可扩展为从配置文件中读取。

- 仅阻止安装，不影响已安装应用的运行。

- 该修改位于系统服务层，需重新编译系统或替换对应服务组件。
